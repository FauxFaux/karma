...
<?
	...
	else
	{
		...
		$res = mysql_query('SELECT `string`,`up`,`down`,' . (!isset($_GET{'real'}) ?  '`value`' : '(`up` - `down`)') . ' AS `mevalue` FROM `_objectdb_plugins_karma_karmaobject` HAVING `mevalue` != 0 ORDER BY `' . $sort . '` DESC,`string` ASC');
?>
	...
	<p>The source is world readable - please suggest improvements.</p>
	<table>
