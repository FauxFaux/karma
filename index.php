...
<?
	...
	if ($reasonspage)
		echo '	<title>Compsoc (Unofficial!) Karma Awards - Reasons for ' . $_GET{'reasons'} . '</title>';
	else
	{
?>
	<title>Compsoc (Unofficial!) Karma Awards</title>
	<script type="text/javascript"><!--
		function showit(item, url)
		{
			document.getElementById("l" + item).style.display='none';
			document.getElementById("r" + item).innerHTML='<iframe src="?reasons=' + url + '" />';
		}
		// -->
	</script>
<?
...
<?
	...
	if ($reasonspage)
	{
		...
		{
			$query='SELECT `reason` FROM `_objectdb_plugins_karma_karmareasonobject` WHERE `string` = "' . mysql_real_escape_string(stripslashes($_GET{'reasons'})) . '" AND `direction` = ';
...
<?
		...
		while ($row = mysql_fetch_assoc($res))
		{
			if ($prevval != $row[$sort])
				$rank++;
			$prevval = $row[$sort];
			echo '		<tr' . ($even ? ' class="even"' : '') . "><td>$rank</td><td>" . (isset($images[strtolower($row['string'])]) ? '<img src="images/' . $images[strtolower($row['string'])] . '" alt="' . $row['string'] . '" />' : '' ) . '</td><td>';
			echo str_replace('_', ' ', htmlentities($row['string']));
			if (isset($hasreasons[strtolower($row['string'])]))
				echo '<p id="l' . md5($row['string']) . '"> ..with <a href="?reasons=' . urlencode($row['string']) . '" onclick="javascript:showit(\'' . md5($row['string']) . '\',\'' . urlencode($row['string']) . '\'); return false;">' . $hasreasons[strtolower($row['string'])] . ' reason' . ($hasreasons[strtolower($row['string'])] == 1 ? '' : 's') . ' &raquo;</a></p><div id="r' . md5($row['string']) . '" />';
