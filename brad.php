...
<?
	...
	if ($reasonspage)
	{
		...
		{
			$query='SELECT `reason` FROM `_objectdb_plugins_brad_bradreasonobject` WHERE `string` = "' . mysql_real_escape_string($_GET{'reasons'}) . '" AND `direction` = ';
			$res = mysql_query($query . $in);
			if (mysql_num_rows($res))
			{
				echo "<h2>..has $mes brad...</h2><ul>";
	...
	else
	{
		...
		$res = mysql_query('SELECT COUNT( `string` ) AS cnt, `string` FROM `_objectdb_plugins_brad_bradreasonobject` GROUP BY `string`');
		$hasreasons = array();
		...
		$res = mysql_query('SELECT `string`,`up`,`down`,' . $value . ' AS `mevalue` FROM `_objectdb_plugins_brad_bradobject` HAVING abs(`mevalue`) > 3 ORDER BY `' . $sort . '` DESC,`string` ASC');
?>
